...
=======
            ...
            if (_eng) {
                ...
                std::shared_ptr<CameraTracking> cameraTrackingPtr = std::static_pointer_cast<CameraTracking>(_eng->getModule("CameraTracking"));
                if(cameraTrackingPtr != nullptr && cameraTrackingPtr->isInitDone) {
                    std::shared_ptr<PoseEstimationRokid> poseEstimationRokidPtr = std::static_pointer_cast<PoseEstimationRokid>(
                            _eng->getModule("PoseEstimationRokid"));
                    if (poseEstimationRokidPtr != nullptr) {
                        std::vector<glm::mat4> &joc = poseEstimationRokidPtr->get_joint_loc();
                        Rendering->joc = joc;
                    }
                    std::shared_ptr<CollisionDetection> collisionDetectionPtr = std::static_pointer_cast<CollisionDetection>(
                            _eng->getModule("CollisionDetection"));
                    ...
                    if (frameDataPtr) {
                        ...
                        if (!isLoadMap) {
                            ...
                            Rendering->view =
                                    frameDataPtr->relocMatrix * view * frameDataPtr->transformCG;
                        }
