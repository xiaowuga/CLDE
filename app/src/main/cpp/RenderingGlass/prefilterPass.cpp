...
void PrefilterPass::initShader() {
    ...
    glGenTextures(2, mPrefilterMaps);

    glBindTexture(GL_TEXTURE_CUBE_MAP, mPrefilterMaps[0]);
    ...
    glBindTexture(GL_TEXTURE_CUBE_MAP, mPrefilterMaps[1]);
    for (unsigned int i = 0; i < 6; ++i)
    {
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB16F, 128, 128, 0, GL_RGB, GL_FLOAT, nullptr);
    }
    ...
    for (unsigned int mip = 0; mip < maxMipLevels; ++mip)
    {
        ...
        for (unsigned int i = 0; i < 6; ++i)
        {
            ...
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, mPrefilterMaps[0], mip);
    ...
    for (unsigned int mip = 0; mip < maxMipLevels; ++mip)
    {
        ...
        {
            mShader.setUniformMat4("view", captureViews[i]);
            glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, mPrefilterMaps[1], mip);
